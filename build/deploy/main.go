...
func deploy() error {
	...
	if siteURL != "" {
		...
		if adminUsername != "" && adminPassword != "" {
			...
			_, _, resp := client.Login(context.Background(), adminUsername, adminPassword)
			if resp.Error() != "" {
				return errors.Errorf("failed to login as %s: %s", adminUsername, resp.Error())
			}
...
func uploadPlugin(client *model.Client4, pluginID, bundlePath string) error {
	...
	_, _, resp := client.UploadPluginForced(context.Background(), pluginBundle)
	if resp.Error() != "" {
		return errors.Errorf("failed to upload plugin bundle: %s", resp.Error())
	}

	log.Print("Enabling plugin.")
	_, resp = client.EnablePlugin(context.Background(), pluginID)
	if resp.Error() != "" {
		return errors.Errorf("Failed to enable plugin: %s", resp.Error())
	}
