...
This plugin allows you to censor profanity on your Mattermost server.  The plugin will check all messages before they are posted to any channel for matches against the "Bad Words List" that is configured.  Any word matches that a user tries to submit as a message, will be replaced with a series of "*" to replace the characters of that word.    
